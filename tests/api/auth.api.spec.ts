...
test.describe("POST /auth/sign-in", () => {
  test("valid credentials return 200 and an accessToken string",
    { tag: "@smoke" },
    async ({ request }) => {
...
test.describe("GET /auth/me", () => {
  test("valid token returns 200 with id and username, no password field",
    { tag: "@smoke" },
    async ({ request, authToken }) => {
      ...
      await test.step("send request with valid token", async () => {
        response = await request.get(ME_URL, { headers: authHeaders(authToken) });
      });
...
test.describe("GET /auth/users", () => {
  ...
  test("returns an array of users with id and username, no password",
    { tag: "@smoke" },
    async ({ request, authToken }) => {
      let response: Awaited<ReturnType<typeof request.get>>;

      await test.step("fetch users list", async () => {
        response = await request.get(USERS_URL, { headers: authHeaders(authToken) });
      });
  ...
  test("all five seed users are present", async ({ request, authToken }) => {
    const response = await request.get(USERS_URL, { headers: authHeaders(authToken) });
